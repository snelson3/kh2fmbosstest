...
class CodeGen:
    ...
    def apply_inf_hp(self):
        self.apply_cb_code(INF_HP_CODE, "Have infinite HP (can still get one shot without OM/SC)")
    def apply_all_forms(self):
        self.apply_cb_code(R2_UNLOCK_DRIVES_CODE, "Hold R2 for all drives") 
    def apply_all_items(self):
        self.apply_cb_code(HAVE_ALL_ITEMS_CODE, "Get all items+Max XP on new game")
    def test(self):
        self.apply_cb_code(TEST_LOC)
